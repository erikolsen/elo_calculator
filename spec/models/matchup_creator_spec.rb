...
describe MatchupCreator do
  ...
  let(:matchup) { Matchup.create primary_id: primary_id, secondary_id: secondary_id, series_max: 5 }
  let(:game_results){
    { '1' => secondary_id,
      '2' => secondary_id,
      '3' => primary_id,
  ...
  context '#save' do
    context 'with 1 games' do
      let(:matchup) { Matchup.create primary_id: primary_id, secondary_id: secondary_id, series_max: 1 }
      let(:game_results){
        { '1' => secondary_id }
      }
      let(:params) { { game_results: game_results, matchup_id: matchup.id } }
      ...
      context 'valid matchup' do
        it 'creates the games' do
          subject.save
          expect(subject.matchup.winner).to eq player_2
        ...
        it 'creates games for each game in matchup' do
          subject.save
          expect(subject.matchup.games.count).to be 1
    ...
    context 'with 3 games' do
      let(:matchup) { Matchup.create primary_id: primary_id, secondary_id: secondary_id, series_max: 3 }
      let(:game_results){
        { '1' => secondary_id,
          '2' => secondary_id,
          '3' => primary_id }
      ...
      context 'valid matchup' do
        ...
        it 'creates games for each game in matchup' do
          subject.save
          expect(subject.matchup.games.count).to be 3
          expect(subject.matchup.games[0].winner.id).to be secondary_id
    ...
    context 'with 5 games' do
      ...
      context 'valid matchup' do
        ...
        it 'creates games for each game in matchup' do
          ...
          expect(subject.matchup.games[0].winner.id).to be primary_id
          expect(subject.matchup.games[1].winner.id).to be secondary_id
          expect(subject.matchup.games[2].winner.id).to be primary_id
          expect(subject.matchup.games[3].winner.id).to be secondary_id
          expect(subject.matchup.games[4].winner.id).to be secondary_id
        end
      end
    end
  end
  ...
  context 'invalid matchup' do
    context 'too many games in matchup' do
      let(:too_many_games_matchups){ {  '1' => '2',
                                        '2' => secondary_id,
                                        '3' => secondary_id,
                                        '4' => primary_id,
                                        '5' => primary_id,
    ...
    context 'too many games in matchup' do
      let(:no_winner_matchups){ {  '1' => secondary_id,
                                   '2' => secondary_id,
                                   '3' => primary_id,
                                   '4' => primary_id} }
